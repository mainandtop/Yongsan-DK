...
module.exports = async function handler(req, res) {
  ...
  const siteName = process.env.SITE_NAME || 'DK밸리뷰 용산';
  ...
  const html = `
    <div style="font-family:'Apple SD Gothic Neo',sans-serif;max-width:560px;margin:0 auto;border:1px solid #eee;border-radius:10px;overflow:hidden">
      <div style="background:#1B2A4A;padding:24px 28px;color:#fff">
        <h2 style="margin:0;font-size:18px;letter-spacing:1px">${esc(siteName)} 방문예약</h2>
      ...
      <div style="padding:26px 28px;background:#fff">
        <table style="width:100%;border-collapse:collapse;font-size:14px">
          ...
          <tr style="border-bottom:1px solid #f0f0f0">
            <td style="padding:12px 0;color:#1B2A4A;font-weight:700">관심 상품</td>
            <td style="padding:12px 0">${esc(unit_type || '미선택')}</td>
          </tr>
          <tr>
            <td style="padding:12px 0;color:#1B2A4A;font-weight:700">방문예약</td>
      ...
      <div style="padding:14px 28px;background:#F4F2EA;font-size:11px;color:#999">
        본 메일은 ${esc(siteName)} 방문예약 폼을 통해 자동 발송되었습니다.
      </div>
    </div>`;
  ...
  try {
    ...
    for (const recipient of receivers) {
      await transporter.sendMail({
        from: `"${siteName} 방문예약" <${process.env.EMAIL_USER}>`,
        to: recipient,
        subject: visitLabel
          ? `[방문예약] ${visitLabel} — ${from_name} / ${phone} — ${siteName}`
          : `[방문예약] ${from_name} / ${phone} — ${siteName}`,
        html,
      });
    }
